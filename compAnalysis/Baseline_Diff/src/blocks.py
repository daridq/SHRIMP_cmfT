...
class UNet(nn.Module):
    ...
    """
    def __init__(self, in_dim, out_dim, embed_dim, dim_scales, no_clp=False):
        super().__init__()

        self.no_clp = no_clp
        self.init_embed = nn.Conv2d(in_dim, embed_dim, 1)  # > b, embed_dim, h, w
        self.time_embed = PositionalEmbedding(embed_dim)
        self.lead_time_embed = PositionalEmbedding(embed_dim)  # Add lead time
        self.time_fusion = nn.Sequential(  # Add lead time
        ...
        for idx, (in_c, out_c) in enumerate(zip(
            ...
            self.down_blocks.extend(nn.ModuleList([
                ...
                BasicBlock(in_c, in_c, embed_dim),
                nn.Conv2d(in_c, out_c, 3, 2, 1) if not is_last else nn.Conv2d(in_c, out_c, 1),  # 3*3 2 feature /2
            ]))
    ...
    def forward(self, x, t, lt):  # Add lead time
        x = x if self.no_clp else torch.clamp(x, min=-1, max=1)  # Clamp x, make it easier to learn?
        x = self.init_embed(x)
        t = self.time_embed(t)
        lt = self.lead_time_embed(lt)  # Add lead time
        #concated_t = self.time_fusion(torch.cat([t, lt], dim=-1))  # Add lead time to original t
        concated_t = t
